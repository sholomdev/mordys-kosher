// const locations = [
//   {
//     name: 'McCormick Place',
//     slug: 'mccormick-place',
//     city: 'Chicago',
//     headerImage: '/images/headerBGs/mccormick-place-bg.jpg',
//     cartLocation: 'Determined as the event approaches',
//     hours: 'During shows, expos & conventions',
//     phone: '312-825-1113',
//     manager: 'Mordy Siegal',
//     operatedBy: 'Kosher Concessions LLC',
//     kashrut: '',
//     detailsImage: '/images/mccormick-details.jpg',
//     aboutImage: '/images/mccormick-place.jpg',
//     about: `This is our flagship location, where Mordy launched his first kosher food cart in 2010. Some have even remarked that it is here where one can taste all the flavors of the world. After all, merchants from every corner on earth gather here to peddle their wares.
// \nChicago’s McCormick Place is the largest convention center in North America. As early as 1927, Robert R. McCormick, championed a lakeside convention center for Chicago. Located just minutes from downtown Chicago, it attracts close to three million visitors each year and boasts 2.6 million square feet of exhibit space.
// `,
//     toppingImage: '/images/toppings.jpg',
//     menuItems: [
//       {
//         name: `Kosher Dog`,
//         note: '',
//         price: 7.75,
//         image: '/images/food/hot-dog.jpg',
//       },
//       {
//         name: `Hot Dog Meal`,
//         note: '(includes chips & a drink',
//         price: 13.5,
//         image: '/images/food/hot-dog-meal.jpg',
//       },
//       {
//         name: 'Chili Dog',
//         note: '',
//         price: 9.5,
//         image: '/images/food/chili-dog.jpg',
//       },
//       {
//         name: `Hot Pastrami Sandwich`,
//         note: '',
//         price: 14.75,
//         image: '/images/food/pastrami.jpg',
//       },
//       {
//         name: `Turkey Sandwich`,
//         note: '',
//         price: 13.50,
//         image: '/images/food/turkey.jpg',
//       },
 
//       {
//         name: `Chili Bowl`,
//         note: '',
//         price: 6.75,
//         image: '/images/food/chili.png',
//       },
//       {
//         name: `Chips`,
//         note: '',
//         price: 3,
//         image: '/images/food/chips.jpg',
//       },
//       {
//         name: `Soda`,
//         note: '',
//         price: 5,
//         image: '/images/food/pop.jpg',
//       },
//       {
//         name: `Water`,
//         note: '',
//         price: 5,
//         image: '/images/food/water.jpg',
//       },
//     ],
//   },
//   {
//     name: 'Soldier Field',
//     city: 'Chicago',
//     slug: 'soldier-field',
//     headerImage: '/images/headerBGs/soldier-field.jpg',
//     cartLocation:
//       'Bears games, section 325, south end zone. For other events please contact us.',
//     hours:
//       'When gates open (usually 2 hours before an event) until you are full. Not open Friday night and Saturday.',
//     phone: '312-825-1113',
//     manager: 'Mordy Siegal',
//     operatedBy: 'Kosher Concessions LLC',
//     kashrut: 'Mashgiach temidi at the stand',
//     detailsImage: '/images/soldier-field-chicago.jpg',
//     aboutImage: '/images/soldier-field-1.jpg',
//     about: `Celebrate every Bears touchdown and pick-six with us behind the south end-zone. Our red hot grill is fired up for our complete Chicago style topping bar.
// \nSoldier Field opened in 1924 and was designated a memorial to U.S. soldiers who fell in combat. It is home to the Chicago Bears and is the second-oldest stadium in the NFL. The stadium also hosts concerts, rodeos, open-air operas, and soccer. `,
//     toppingImage: '/images/toppings.jpg',
//     menuItems: [
//       {
//         name: `Mordy's Hot Dog`,
//         note: '',
//         price: 11,
//         image: '/images/food/hot-dog.jpg',
//       },
      
//       {
//         name: `'85 Pastrami Dog'`,
//         note: '',
//         price: 14,
//         image: '/images/food/pastrami-dog.jpg',
//       },
//       {
//         name: `The Butkus Burger`,
//         note: '',
//         price: 17,
//         image: '/images/food/burger.jpg',
//       },
//       {
//         name: `Club Dub Hot Pastrami`,
//         note: '',
//         price: 16.5,
//         image: '/images/food/pastrami.jpg',
//       },
//       {
//         name: `Polish Sausage`,
//         note: '',
//         price: 12.5,
//         image: '/images/food/polish.jpg',
//       },      {
//         name: `Water`,
//         note: '',
//         price: 6.75,
//         image: '/images/food/water.jpg',
//       },
//     ],
//   },
//   {
//     name: 'Wells Fargo Center',
//     city: 'Philadelphia',
//     slug: 'wells-fargo-center',
//     headerImage: '/images/headerBGs/wells-fargo.jpg',
//     cartLocation:
//       '3601 S Broad St, Philadelphia, PA 19148, Section 201 - center court on the 2nd level (Mezzanine).',
//     hours:
//       'When gates open (usually 90 minutes before an event) until you are full. Not open Friday and Saturday.',
//     phone: '215-917-0209',
//     manager: '',
//     contact: 'David Kushner',
//     email: 'philly@mordyskosher.com',
//     operatedBy: 'Kosher Concessions Philly LLC',
//     kashrut: 'Community Kashrus of Greater Philadelphia, The Keystone-K',
//     detailsImage: '/images/wells-fargo.jpg',
//     aboutImage: '/images/wells-fargo-floor.jpg',
//     about: `Mordy’s Kosher and Philly have a lot in common: Food, sports, and history. Celebrate the home that Dr. J and Sir Charles Barkley built, with our famous soft pretzel in one hand and Mordy’s Hot Dog in the other.
//  \nThe Wells Fargo Center is one of the busiest arenas in the world and home to the Philadelphia 76ers and Philadelphia Flyers. The arena has also hosted the 2000 Republican National Convention and the 2016 Democratic National Convention.
// `,
//     toppingImage: '/images/toppings.jpg',
//     menuItems: [
//       {
//         name: `Kosher Hot Dog`,
//         note: '',
//         price: 9.99,
//         image: '/images/food/hot-dog.jpg',
//       },
//       {
//         name: `Pastrami Dog`,
//         note: '',
//         price: 13.29,
//         image: '/images/food/pastrami-dog.jpg',
//       },

//       {
//         name: `Hot Pastrami Sandwich`,
//         note: '',
//         price: 17.19,
//         image: '/images/food/pastrami.jpg',
//       },

//       {
//         name: `Soft Pretzel`,
//         note: '',
//         price: 6.35,
//         image: '/images/food/pretzel.jpg',
//       },
//       {
//         name: `Knish`,
//         note: '',
//         price: 6.99,
//         image: '/images/food/knish.jpg',
//       },
//       {
//         name: `Pastrami Egg Roll`,
//         note: '',
//         price: 9.99,
//         image: '/images/food/egg-roll.jpg',
//       }
//     ],
//   },
//   {
//     name: 'Citizens Bank Park',
//     slug: 'citizens-bank-park',
//     city: 'Philadelphia',
//     headerImage: '/images/headerBGs/citizens-bank.jpg',
//     cartLocation:
//      '1 Citizens Bank Way, Philadelphia, PA 19148, Main Councourse behind home plate - section 124',
//     hours:
//       'When gates open (usually 90 minutes before a Phillies home game) until you are full. Not open Friday  and Saturday.',
//     phone: '215-917-0209',
//     manager: '',
//     contact: 'David Kushner',
//     email: 'philly@mordyskosher.com',
//     operatedBy: 'Kosher Concessions Philly LLC',
//     kashrut: 'Community Kashrus of Greater Philadelphia, The Keystone-K',
//     detailsImage: '/images/citizens-details.jpg',
//     aboutImage: '/images/citizens-about.jpg',
//     about: `Mordy’s Kosher and Philly have a lot in common: Food, sports, and history. Celebrate the home that Richie Ashburn and Mike Schmidt built, with our famous soft pretzel in one hand and Mordy’s Hot Dog in the other.
//  \nCitizens Bank Park is one of the busiest stadiums in the world and home to the Philadelphia Phillies. The stadium has also earned a global reputation as one of the nicest tadiums in the world with amazing views from every seat.
// `,
//     toppingImage: '/images/toppings.jpg',
//     menuImage: '/images/citizens-menu.jpg',
//     flyerImage: '/images/citizens-flyer.jpg',
//   },
//   {
//     name: 'United Center',
//     slug: 'united-center',
//     city: 'Chicago',
//     headerImage: '/images/headerBGs/united-center-2.jpg',
//     cartLocation: 'Secion 120',
//     hours:
//       'During Bulls and Blackhawks games. Open 90 minutes before gametime. Not open friday night and saturday.',
//     phone: '312-825-1113',
//     manager: 'Mordy Siegal',
//     operatedBy: 'Kosher Concessions  LLC',
//     kashrut: 'CRC',
//     detailsImage: '/images/united-center-2.jpg',
//     aboutImage: '/images/united-center.jpg',
//     about: ``,
//     toppingImage: '/images/toppings.jpg',
//     menuItems: [
//       {
//         name: `Kosher Dog`,
//         note: '',
//         price: 9,
//         image: '/images/food/hot-dog.jpg',
//       },
//       {
//         name: `Pastrami Dog'`,
//         note: '',
//         price: 12.5,
//         image: '/images/food/pastrami-dog.jpg',
//       },
//       {
//         name: `Hot Pastrami Sandwich`,
//         note: '',
//         price: 15.5,
//         image: '/images/food/pastrami.jpg',
//       },
//       {
//         name: `Polish Sausage`,
//         note: '',
//         price: 9.75,
//         image: '/images/food/polish.jpg',
//       },
//       {
//         name: `Water`,
//         note: '',
//         price: 6,
//         image: '/images/food/water.jpg',
//       },
//       {
//         name: `Pop`,
//         note: '',
//         price: 7.25,
//         image: '/images/food/pop.jpg',
//       },
//     ],
//   },
// ];

// const mediaData = [
//   {
//     id: '1',
//     image: '/images/media/inquirer-logo-white.png',
//     title: 'Kosher food joins the lineup for Phillies games at Citizens Bank Park',
//     description:
//       'Phillies fans can visit Kosher Grill for hot pastrami on rye, hot dogs with various toppings, garlic beef sausage, potato knishes, pastrami and cabbage egg rolls, and pretzels.',
//     link: 'https://www.inquirer.com/food/restaurants/kosher-food-at-phillies-citizens-bank-park-20240617.html',
//   },
//   {
//     id: '2',
//     image: '/images/media/JE-Logo.webp',
//     title: 'Kosher Food Stand Opens at Citizens Bank Park',
//     description:
//       'On June 17, a kosher food stand opened on the third base side at Citizens Bank Park. The stand combines ballpark favorites such as hot dogs and pretzels with kosher staples such as hot pastrami on rye and potato knishes...',
//     link: '/images/media/exponent.pdf',
//   },

 
//   {
//     id: '4',
//     image: '/images/media/forward-logo.webp',
//     title: 'Serving Kosher Food to the Hungry DNC Masses',
//     description:
//       'Though food options abound at the Democratic National Convention in Philadelphia, Mordy Siegal is committed to providing convenient kosher food to the hungry masses.',
//     link: 'https://forward.com/scribe/346158/serving-kosher-food-to-the-hungry-dnc-masses/',
//   },
//   {
//     id: '6',
//     image: '/images/media/crains-logo.webp',
//     title: 'Bears games offer kosher food option',
//     description:
//       'Add kosher food to the new concessions options at Chicago Bears games this year. The new offering is the first-ever kosher food option at Soldier Field.',
//     link: 'https://www.chicagobusiness.com/article/20140922/BLOGS04/140929998/chicago-bears-games-now-offer-kosher-food-option',
//   },
//   {
//     id: '6',
//     image: '/images/media/JE-Logo.webp',
//     title: 'New Kosher Vendor Changes the Game at Wells Fargo Center',
//     description:
//       'Kosher Grill turns out everything from hot pretzels to Polish sausage, hot dogs, pastrami sandwiches and, of course, a must-have pastrami hot dog.',
//     link: '',
//   },

// ];

// export { locations, mediaData };

// Change 'mordys-kosher' to your actual Cloudinary Cloud Name
const CLOUDINARY_BASE = 'https://res.cloudinary.com/dsjy8ydj7/image/upload/mordys-kosher';
const locations = [
  {
    name: 'McCormick Place',
    slug: 'mccormick-place',
    city: 'Chicago',
    headerImage: `${CLOUDINARY_BASE}/headerBGs/mccormick-place-bg.jpg`,
    cartLocation: 'Determined as the event approaches',
    hours: 'During shows, expos & conventions',
    phone: '312-825-1113',
    manager: 'Mordy Siegal',
    operatedBy: 'Kosher Concessions LLC',
    kashrut: '',
    detailsImage: `${CLOUDINARY_BASE}/mccormick-details.jpg`,
    aboutImage: `${CLOUDINARY_BASE}/mccormick-place.jpg`,
    about: `This is our flagship location, where Mordy launched his first kosher food cart in 2010. Some have even remarked that it is here where one can taste all the flavors of the world. After all, merchants from every corner on earth gather here to peddle their wares.
\nChicago’s McCormick Place is the largest convention center in North America. As early as 1927, Robert R. McCormick, championed a lakeside convention center for Chicago. Located just minutes from downtown Chicago, it attracts close to three million visitors each year and boasts 2.6 million square feet of exhibit space.
`,
    toppingImage: `${CLOUDINARY_BASE}/toppings.jpg`,
    menuItems: [
      {
        name: `Kosher Dog`,
        price: 7.75,
        image: `${CLOUDINARY_BASE}/food/hot-dog.jpg`,
      },
      {
        name: `Hot Dog Meal`,
        note: '(includes chips & a drink)',
        price: 13.5,
        image: `${CLOUDINARY_BASE}/food/hot-dog-meal.jpg`,
      },
      {
        name: 'Chili Dog',
        price: 9.5,
        image: `${CLOUDINARY_BASE}/food/chili-dog.jpg`,
      },
      {
        name: `Hot Pastrami Sandwich`,
        price: 14.75,
        image: `${CLOUDINARY_BASE}/food/pastrami.jpg`,
      },
      {
        name: `Turkey Sandwich`,
        price: 13.50,
        image: `${CLOUDINARY_BASE}/food/turkey.jpg`,
      },
      {
        name: `Chili Bowl`,
        price: 6.75,
        image: `${CLOUDINARY_BASE}/food/chili.png`,
      },
      {
        name: `Chips`,
        price: 3,
        image: `${CLOUDINARY_BASE}/food/chips.jpg`,
      },
      {
        name: `Soda`,
        price: 5,
        image: `${CLOUDINARY_BASE}/food/pop.jpg`,
      },
      {
        name: `Water`,
        price: 5,
        image: `${CLOUDINARY_BASE}/food/water.jpg`,
      },
    ],
  },
  {
    name: 'Soldier Field',
    city: 'Chicago',
    slug: 'soldier-field',
    headerImage: `${CLOUDINARY_BASE}/headerBGs/soldier-field.jpg`,
    cartLocation:
      'Bears games, section 325, south end zone. For other events please contact us.',
    hours:
      'When gates open (usually 2 hours before an event) until you are full. Not open Friday night and Saturday.',
    phone: '312-825-1113',
    manager: 'Mordy Siegal',
    operatedBy: 'Kosher Concessions LLC',
    kashrut: 'Mashgiach temidi at the stand',
    detailsImage: `${CLOUDINARY_BASE}/soldier-field-chicago.jpg`,
    aboutImage: `${CLOUDINARY_BASE}/soldier-field-1.jpg`,
    about: `Celebrate every Bears touchdown and pick-six with us behind the south end-zone. Our red hot grill is fired up for our complete Chicago style topping bar.
\nSoldier Field opened in 1924 and was designated a memorial to U.S. soldiers who fell in combat. It is home to the Chicago Bears and is the second-oldest stadium in the NFL. The stadium also hosts concerts, rodeos, open-air operas, and soccer. `,
    toppingImage: `${CLOUDINARY_BASE}/toppings.jpg`,
    menuItems: [
      {
        name: `Mordy's Hot Dog`,
        price: 11,
        image: `${CLOUDINARY_BASE}/food/hot-dog.jpg`,
      },
      {
        name: `'85 Pastrami Dog'`,
        price: 14,
        image: `${CLOUDINARY_BASE}/food/pastrami-dog.jpg`,
      },
      {
        name: `The Butkus Burger`,
        price: 17,
        image: `${CLOUDINARY_BASE}/food/burger.jpg`,
      },
      {
        name: `Club Dub Hot Pastrami`,
        price: 16.5,
        image: `${CLOUDINARY_BASE}/food/pastrami.jpg`,
      },
      {
        name: `Polish Sausage`,
        price: 12.5,
        image: `${CLOUDINARY_BASE}/food/polish.jpg`,
      },
      {
        name: `Water`,
        price: 6.75,
        image: `${CLOUDINARY_BASE}/food/water.jpg`,
      },
    ],
  },
  {
    name: 'Xfinity Mobile Arena',
    city: 'Philadelphia',
    slug: 'xfinity-mobile-arena',
    headerImage: `${CLOUDINARY_BASE}/headerBGs/xfinity.jpg`,
    cartLocation: '3601 S Broad St, Philadelphia, PA 19148, Section 201 - center court on the 2nd level (Mezzanine).',
    hours: 'When gates open (usually 90 minutes before an event) until you are full. Not open Friday and Saturday.',
    phone: '215-917-0209',
    contact: 'David Kushner',
    email: 'philly@mordyskosher.com',
    operatedBy: 'Kosher Concessions Philly LLC',
    kashrut: 'Community Kashrus of Greater Philadelphia, The Keystone-K',
    kashrutImage: `${CLOUDINARY_BASE}/xfinity-kashrut.jpg`,
    brandingHeader: `${CLOUDINARY_BASE}/kosher-grill-header.jpg`,
    detailsImage: `${CLOUDINARY_BASE}/wells-fargo.jpg`,
    aboutImage: `${CLOUDINARY_BASE}/wells-fargo-floor.jpg`,
    about: `Mordy’s Kosher and Philly have a lot in common: Food, sports, and history. Celebrate the home that Dr. J and Sir Charles Barkley built, with our famous soft pretzel in one hand and Mordy’s Hot Dog in the other.
\nXfinity Mobile Arena is one of the busiest arenas in the world and home to the Philadelphia 76ers and Philadelphia Flyers. The arena has also hosted the 2000 Republican National Convention and the 2016 Democratic National Convention.`,
    toppingImage: `${CLOUDINARY_BASE}/toppings.jpg`,
    flyerImage: `${CLOUDINARY_BASE}/xfinity-flyer.jpg`,
    menuItems: [
      {
        name: `All Beef Kosher Hot Dog`,
        price: 8.99,
        image: `${CLOUDINARY_BASE}/food/hot-dog.jpg`,
      },
      {
        name: `Pastrami Dog`,
        price: 12.99,
        image: `${CLOUDINARY_BASE}/food/pastrami-dog.jpg`,
      },
           {
        name: `Pulled Beef Brisket Sandwich`,
        price: 16.99,
        image: `${CLOUDINARY_BASE}/food/pulled-brisket.jpg`,
      },
      {
        name: `Hot Pastrami Sandwich`,
        price: 18.99,
        image: `${CLOUDINARY_BASE}/food/pastrami.jpg`,
      },
      {
        name: `Soft Pretzel`,
        price: 6.99,
        image: `${CLOUDINARY_BASE}/food/pretzel.jpg`,
      },
      {
        name: `Potato Knish`,
        price: 7.99,
        image: `${CLOUDINARY_BASE}/food/knish.jpg`,
      },      
      {
        name: `Pulled Brisket Knish`,
        price: 10.99,
        image: `${CLOUDINARY_BASE}/food/knish.jpg`,
      },      
    ],
    drinks: [
    { name: 'Beer', note: 'Domestic & Premium selection' },
    { name: 'Bottled Water', note: 'Dasani or SmartWater' },
    { name: 'Bottled Soda', note: 'Coke, Diet Coke, Coke Zero, Sprite, Fanta Orange' }
  ]
  },
  {
    name: 'Citizens Bank Park',
    slug: 'citizens-bank-park',
    city: 'Philadelphia',
    headerImage: `${CLOUDINARY_BASE}/headerBGs/citizens-bank.jpg`,
   cartLocation:
     '1 Citizens Bank Way, Philadelphia, PA 19148, Main Councourse behind home plate - section 124',
    hours:
      'When gates open (usually 90 minutes before a Phillies home game) until you are full. Not open Friday  and Saturday.',

    phone: '215-917-0209',
    contact: 'David Kushner',
    email: 'philly@mordyskosher.com',
    operatedBy: 'Kosher Concessions Philly LLC',
    kashrut: 'Community Kashrus of Greater Philadelphia, The Keystone-K',
        kashrutImage: `${CLOUDINARY_BASE}/cbp-kashrut.jpg`,

    detailsImage: `${CLOUDINARY_BASE}/citizens-details.jpg`,
    aboutImage: `${CLOUDINARY_BASE}/citizens-about.jpg`,
     about: `Mordy’s Kosher and Philly have a lot in common: Food, sports, and history. Celebrate the home that Richie Ashburn and Mike Schmidt built, with our famous soft pretzel in one hand and Mordy’s Hot Dog in the other.
 \nCitizens Bank Park is one of the busiest stadiums in the world and home to the Philadelphia Phillies. The stadium has also earned a global reputation as one of the nicest tadiums in the world with amazing views from every seat.
`,
    toppingImage: `${CLOUDINARY_BASE}/toppings.jpg`,
    menuImage: `${CLOUDINARY_BASE}/citizens-menu.jpg`,
    flyerImage: `${CLOUDINARY_BASE}/citizens-flyer.jpg`,
    brandingHeader: `${CLOUDINARY_BASE}/kosher-grill-header.jpg`,
  },
  {
    name: 'United Center',
    slug: 'united-center',
    city: 'Chicago',
    headerImage: `${CLOUDINARY_BASE}/headerBGs/united-center-2.jpg`,
    cartLocation: 'Section 120',
   hours:
      'During Bulls and Blackhawks games. Open 90 minutes before gametime. Not open friday night and saturday.',

    phone: '312-825-1113',
    manager: 'Mordy Siegal',
    operatedBy: 'Kosher Concessions LLC',
    kashrut: 'CRC',
    detailsImage: `${CLOUDINARY_BASE}/united-center-2.jpg`,
    aboutImage: `${CLOUDINARY_BASE}/united-center.jpg`,
about: `Experience the energy of the "Madhouse on Madison" with Mordy’s Kosher. Located in the heart of the United Center at Section 120, we serve as the premier destination for fans of the Chicago Bulls and Blackhawks alike. Whether you're cheering on a game-winning bucket or a power-play goal, our stand offers the perfect arena-side meal to keep you in the action. We are proud to bring a high-quality, certified kosher tradition to one of the most iconic sports venues in the world.`,
    toppingImage: `${CLOUDINARY_BASE}/toppings.jpg`,
    menuItems: [
      {
        name: `Kosher Dog`,
        price: 9,
        image: `${CLOUDINARY_BASE}/food/hot-dog.jpg`,
      },
      {
        name: `Pastrami Dog'`,
        price: 12.5,
        image: `${CLOUDINARY_BASE}/food/pastrami-dog.jpg`,
      },
      {
        name: `Hot Pastrami Sandwich`,
        price: 15.5,
        image: `${CLOUDINARY_BASE}/food/pastrami.jpg`,
      },
      {
        name: `Polish Sausage`,
        price: 9.75,
        image: `${CLOUDINARY_BASE}/food/polish.jpg`,
      },
      {
        name: `Water`,
        price: 6,
        image: `${CLOUDINARY_BASE}/food/water.jpg`,
      },
      {
        name: `Pop`,
        price: 7.25,
        image: `${CLOUDINARY_BASE}/food/pop.jpg`,
      },
    ],
  },
];

const mediaData = [
  {
    id: '1',
    image: `${CLOUDINARY_BASE}/media/inquirer-logo-white.png`,
    title: 'Kosher food joins the lineup for Phillies games...',
    description: 'Phillies fans can visit Kosher Grill for hot pastrami on rye...',
    link: 'https://www.inquirer.com/food/restaurants/kosher-food-at-phillies-citizens-bank-park-20240617.html',
  },
  {
    id: '2',
    image: `${CLOUDINARY_BASE}/media/JE-Logo.webp`,
    title: 'Kosher Food Stand Opens at Citizens Bank Park',
    description: 'On June 17, a kosher food stand opened...',
    link: '/images/media/exponent.pdf', // Keep local if it's a PDF
  },
  {
    id: '4',
    image: `${CLOUDINARY_BASE}/media/forward-logo.webp`,
    title: 'Serving Kosher Food to the Hungry DNC Masses',
    description: 'Mordy Siegal is committed to providing convenient kosher food...',
    link: 'https://forward.com/scribe/346158/serving-kosher-food-to-the-hungry-dnc-masses/',
  },
  {
    id: '5', // Fixed duplicate ID
    image: `${CLOUDINARY_BASE}/media/crains-logo.webp`,
    title: 'Bears games offer kosher food option',
    description: 'Add kosher food to the new concessions options at Chicago Bears games...',
    link: 'https://www.chicagobusiness.com/article/20140922/BLOGS04/140929998/chicago-bears-games-now-offer-kosher-food-option',
  },
  {
    id: '6',
    image: `${CLOUDINARY_BASE}/media/JE-Logo.webp`,
    title: 'New Kosher Vendor Changes the Game at Wells Fargo Center',
    description: 'Kosher Grill turns out everything from hot pretzels to Polish sausage...',
    link: '',
  },
];

export { locations, mediaData };